...
/*
iSize1肯定是正确的
但iSize2就不一定了，就需要在GetSize运行完成之后才知道
这个时候可以用上constexpr，更加符合常量的定义
*/
```
...
为什么要用这个关键字呢，我认为和auto作用是差不多的，如果给了一个表达式，而这个表达式的类型很复杂，这个时候就可以用decltype来推断出这个表达式的返回类型，然后进行使用。

既然已经有了 auto 关键字，为什么还需要 decltype 关键字呢？因为 auto 并不适用于所有的自动类型推导场景，在某些特殊情况下 auto 用起来非常不方便，甚至压根无法使用，

auto是根据=右边的值来推导类型，decltype是根据表达式来推导类型。
